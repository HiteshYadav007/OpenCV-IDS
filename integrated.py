...
while(True):
    ...
    # Display the FPS
    cv2.putText(annotated_image, 'FPS: {:.2f}'.format(fps), (510, 450), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (0,0, 0),2)

    # Display Time
    cv2.putText(annotated_image, current_time, (310, 20), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 0, 0),1)    
    
    # Display the Room Status
    cv2.putText(annotated_image, 'Room Occupied: {}'.format(str(status)), (10, 20), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (0, 0, 0),2)
    # Show the patience Value
    if initial_time is None:
        text = 'Patience: {}'.format(patience)
    else: 
        text = 'Patience: {:.2f}'.format(max(0, patience - (time.time() - initial_time)))
    cv2.putText(annotated_image, text, (10, 450), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (0, 0, 0) , 2)   
